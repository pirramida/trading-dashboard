...
const WindowManager = () => {
  ...
  const handleResizeStop = (id, direction, ref, delta, position) => {
    console.log(`[WindowManager] Окно ${id} изменило размер: width=${ref.style.width}, height=${ref.style.height}, x=${position.x}, y=${position.y}`);
    setWindows(windows.map(w =>
      w.id === id ? {
        ...w,
        ...
        >
          <div style={{
            ...
            <div style={{
              ...
              <div>
                {window.type.toUpperCase()} - {window.symbol} {window.interval && `(${window.interval}m)`}
              </div>
              <button
